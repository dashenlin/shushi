 ...
 * @return 分页
 */
function wx_pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
	...
	if($num > $perpage) {
		...
		if($curr_page>0) {
			$multipage .= '<ul class="pagination"><li> <a href="'.pageurl($urlrule, $curr_page-1, $array).'" class="a1">'.L('previous').'</a>';
			if($curr_page==1) {
				$multipage .= '<li class="active"><a href="#">1</a></li>';
			} elseif($curr_page>6 && $more) {
				$multipage .= '<li><a href="'.pageurl($urlrule, 1, $array).'">1</a></li><li><a>...</a></li>';
			} else {
		...
		if($curr_page<$pages) {
			if($curr_page<$pages-5 && $more) {
				$multipage .= '<li><a>...</a><li><li><a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a></li><li><a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="a1">'.L('next').'</a></li>';
			} else {
				$multipage .= '<li><a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> <a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="a1">'.L('next').'</a></li>';
			}
		} elseif($curr_page==$pages) {
			$multipage .= '<li  class="active"><a>'.$pages.'</a></li><li><a href="'.pageurl($urlrule, $curr_page, $array).'" class="a1">'.L('next').'</a></li>';
		} else {
		...
		$multipage .= '<li><a href="#">'."共".$num."条".'</a></li></ul>';
	}
	return $multipage;
}
 ...
 * 获取多图数组
 */
function images_json($imgArr){
	$imgArr=rtrim($imgArr, "{");
	$imgArr=ltrim($imgArr, "}");
	$imgArr=stripslashes($imgArr);
	return json_decode($imgArr,true);
}
